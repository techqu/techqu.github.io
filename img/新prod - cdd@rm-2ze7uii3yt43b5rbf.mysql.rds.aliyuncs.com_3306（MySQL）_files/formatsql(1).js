...
var keywords=new Array("begin","end","select","insert","update","delete","replace","drop","create","end","return","delimiter","from","right","left","inner","where","order","group","by","having","set","union","and","or",'limit');
var words = new Array();
var wordscnt=0;
...
function buildwords(istr){
	...
	while (i<istr.length) {
		if (curr_status=="string") {
        ...
        }else if (curr_status=="commentblock") {
            ...
            tempstr=istr.substring(pos1,i);
            addWords(curr_status,tempstr,"commentblock");
            pos1=i;
            i++;
        }else if ((istr.charAt(i)=="-") && (istr.charAt(i+1)=="-")) {
            tempstr=istr.substring(pos1,i);
...
function formatwords() {
	...
	for (i=0;i<wordscnt;i++) {
		...
		}else if (words[i][0]=="commentblock") {
      		tempstr = tempstr + "\n" + words[i][1] + "\n" + spacestr;
		}else if (words[i][1]==","){
			if (tempstr=="") {
				tempstr = tempstr + ',';
